...
def get_data_set(name="train", cifar=10):
    ...
    if cifar == 10:
        ...
        return x, dense_to_one_hot(y, num_classes=10), y
    
    if cifar == 100:
        f = open('./data_set/'+folder_name+'/meta', 'rb')
        datadict = pickle.load(f, encoding='latin1')
        f.close()
        l = datadict['coarse_label_names']
        l_fine = datadict['fine_label_names']
        # print(len(l))
        # print(l.iloc[0:30, :])
        # print(l.columns())
        if name is "train":
            ...
            x = x.transpose([0, 2, 3, 1])
            x = x.reshape(-1, 32*32*3)
            # x = (x-tf.reduce_max(x))/(tf.reduce_max(x)-tf.reduce_min(x))
            ...
            x = x.reshape(-1, 32*32*3)
            # x = (x-tf.reduce_max(x))/(tf.reduce_max(x)-tf.reduce_min(x))
        return x, dense_to_one_hot(y, num_classes=100), y
